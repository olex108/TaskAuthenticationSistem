...
from identity.models import Role, Permission, RolePermission
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            permissions_data = [
                ...
                {
                    'code': 'mock:view_analytics',
                    'description': 'Access to view business mock analytics endpoints'
                },
                {
                    'code': 'mock:edit_data',
            ...
            RolePermission.objects.get_or_create(role=roles_pool['administrator'],
                                                 permission=permissions_pool['admin:access'])
            RolePermission.objects.get_or_create(role=roles_pool['administrator'],
                                                 permission=permissions_pool['mock:view_analytics'])
            RolePermission.objects.get_or_create(role=roles_pool['administrator'],
                                                 permission=permissions_pool['mock:edit_data'])

            # Moderator can view analytics and edit business data, but cannot touch admin panel
            RolePermission.objects.get_or_create(role=roles_pool['moderator'],
                                                 permission=permissions_pool['mock:view_analytics'])
            RolePermission.objects.get_or_create(role=roles_pool['moderator'],
                                                 permission=permissions_pool['mock:edit_data'])
