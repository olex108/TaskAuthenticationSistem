...
from identity.models import Role, User, UserRole
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        user = User.objects.create(
            email=email, password_hash=hashed_password, first_name="System", last_name="Admin", is_active=True
        )
        ...
        if not admin_role:
            self.stdout.write(
                self.style.ERROR(
                    f"Critical Error: Role '{role_name}' not found. " "Please run 'seed_roles' command first."
                )
            )
            return
